...
Centaur supports a first-class ordered repo-cache overlay model, so
organizations can extend the base repo without forking or relying on filesystem
overlayfs. A common deployment keeps the base repo and an external overlay
...
The Helm chart supports ordered overlays through `overlays.sources`.
repo-cache checks out each source; api-rs reads tools and workflows from
`/var/lib/centaur/repos/...`, and sandboxes read the same revisions from
`/home/agent/github/...`. Use chart-level `overlay.systemPrompt` only for small
prompt additions. Existing deployments can continue using `overlay.image.*`
while they migrate remaining prompt, harness, and persona assets onto
repo-cache-backed overlay sources.
...
| Primitive | Purpose |
|-----------|---------|
| `ctx.step(name, fn)` | Execute *fn* exactly once; return cached result on replay. |
| `ctx.sleep_for(name, seconds)` | Suspend the run for a duration; checkpoint + resume automatically. |
...
async def handler(inp: Input, ctx: WorkflowContext) -> dict[str, Any]:
    ...
    await ctx.sleep_for("pause", 300)
    result = await ctx.run_agent(f"Summarize: {greeting['msg']}")
...
- **External events and child workflows**: the API has run/event surfaces, but
  the staged Python host should not advertise unsupported v1 context helpers.
