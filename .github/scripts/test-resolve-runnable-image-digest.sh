...
MOCK_INDEX_JSON="$(jq --argjson duplicate "$(jq -c '.manifests[0]' <<<"$MOCK_INDEX_JSON")" \
...
MOCK_INDEX_JSON="$(jq -cn '{mediaType:"application/vnd.oci.image.manifest.v1+json"}')"
export MOCK_INDEX_JSON
