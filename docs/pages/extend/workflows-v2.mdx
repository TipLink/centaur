...
Keep state-changing workflows outside sandbox workflow API grants. Deployment
overlays own their domain schemas, transition rules, provider mappings, and
workflow code. Both the API-to-Slackbot egress policy and the Slackbot ingress
policy must allow the authenticated internal callback.
