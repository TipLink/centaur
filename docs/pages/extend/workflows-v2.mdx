...
## Trusted Slack transport for work-item workflows

Deployments can enable an incident/chore slash-command ingress with
`WORK_ITEMS_ENABLED=true` and `SLACK_COMMAND_NAME=/centaur` on Slackbot v2.
The signed request queues the deployment-provided `work_item_command` workflow;
malformed commands receive a private example without creating a run. Commands
require a quoted description, Slack owner mention, and a duration or explicit
ISO timestamp. The creator/channel identity comes only from the signed payload.
...
To add commands, supply another `SlackCommandDefinition` in the `slashCommands`
configuration assembled by `services/slackbotv2/src/server.ts`. Each definition
declares a unique name, title, description, search keywords, enablement, form
fields, text/form parsers, and trusted workflow name. `work-item-commands.ts`
contains the incident/chore definitions; `slack-command-registry.ts` owns the
shared UI schema and search. The router in `slack-commands.ts` requires no new
route or feature-specific branches. Keep business authorization and durable
effects in the target workflow; registry entries are trusted deployment code,
not sandbox-provided definitions.
...
the command registry may enqueue, such as `work_item_command`. The existing
...
structured work-item context through the ordinary session API. It does not
execute an agent turn.

Keep these state-changing workflows outside sandbox workflow API grants.
The deployment overlay owns its state schema, transition rules, outbox, Linear
mapping, and workflow code. Both the API-to-Slackbot egress policy and the
Slackbot ingress policy must allow the authenticated internal callback.

Slackbot v2 can forward `/api/webhooks/work-items-linear` unchanged to api-rs,
where the configured workflow webhook verifies the provider HMAC before enqueue.
Configure the public edge to expose only that exact POST route. External webhook
payloads must be reconciled against current provider state before applying a
transition.
