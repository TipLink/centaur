...
- `type = "brokered_token"` routes OAuth2 refresh-token rotation through the
  ...
  enabled) and more than one proxy shares the credential. The entry is only the
  consumer side: `name` identifies the injected secret, `hosts` defines its
  request rules, and optional `credential` names the Console broker credential
  by foreign ID (defaulting to `name`). Create that credential separately in
  the Console or with `centaur-perms broker create`. Optional `inject_header`
  and `inject_formatter` override the default Bearer authorization injection.
  Legacy `fields`, `token_endpoint`, and `scopes` keys are ignored and should
  not be declared.
...
- `type = "pg_dsn"` is for Postgres. iron-proxy resolves the real upstream DSN,
  ...
  `name`; `database` must match the upstream database name. Optional `settings`
  pin Postgres session variables. Use
  `value_from = { requester_principal_field = "slack_user_id" }` when a policy
  needs the verified requester for the current turn. The selector resolves to
  an empty string when no requester is bound and never falls back to the
  conversation principal. Open a new database connection for each turn because
  pinned settings are installed when the Postgres session starts.
