...
database = "analytics"
settings = [
  { name = "centaur.slack_user_id", value_from = { requester_principal_field = "slack_user_id" } }
]
```
...
points at iron-proxy instead of the raw upstream DSN. The optional requester
selector above is fail-closed: it reads only the verified requester bound to the
current turn and yields an empty value when there is none. Clients must open a
new Postgres connection per turn so the newly pinned identity takes effect. Use
`oauth_token` when iron-proxy should resolve OAuth credential fields, exchange
them at a token endpoint, and inject a short-lived bearer token for matching API
hosts.
