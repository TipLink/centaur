...
| `overlays.sources` | Ordered repo-cache-backed overlay repos for tools, workflows, skills, and opt-in Slackbot extensions; `slackbotSubdir` points to a directory containing `index.mjs` and requires `ref` to be a full commit SHA. |
...
| Env var | Set from | Controls |
| --- | --- | --- |
| `SLACK_SIGNING_SECRET` | `secretManager.existingSecretName` when `slackbotv2.mode=webhook`; local bootstrap reads shell env. | Required in webhook mode for Slack request signature verification. Not required for native Socket Mode. |
| `SLACK_APP_TOKEN` | `secretManager.existingSecretName` when `slackbotv2.mode=socket`. | Required in Socket Mode for the Chat SDK connection. |
...
| `SLACKBOTV2_MODE` | `slackbotv2.mode`. | Selects `webhook` (default) or Chat SDK `socket` event transport. Socket mode requires `SLACK_APP_TOKEN`; extensions register before the adapter connects. |
...
| `SLACKBOTV2_EXTENSION_MODULES` | Chart-rendered from `overlays.sources[*].slackbotSubdir`. | Ordered JSON descriptors containing each trusted ESM module path, repo path, and immutable revision. Slackbot verifies repo-cache `HEAD`, then loads modules before listening. Each module exports an `extension` manifest and `register(context)`. |
